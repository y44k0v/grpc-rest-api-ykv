...
func (oq *OrderQuery) QueryProducts() *ProductQuery {
...
func (oq *OrderQuery) WithProducts(opts ...func(*ProductQuery)) *OrderQuery {
	query := (&ProductClient{config: oq.config}).Query()
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withProducts; query != nil {
		if err := oq.loadProducts(ctx, query, nodes,
			func(n *Order) { n.Edges.Products = []*Product{} },
			func(n *Order, e *Product) { n.Edges.Products = append(n.Edges.Products, e) }); err != nil {
			return nil, err
		}
	}
...
func (oq *OrderQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Order, init func(*Order), assign func(*Order, *Product)) error {
	...
	query.Where(predicate.Product(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.ProductsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.order_products
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_products" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_products" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
